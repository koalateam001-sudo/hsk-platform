...
- Proxy tetap memproteksi seluruh `/dashboard/*`
...
- [x] REQ-01: User yang belum login tidak bisa akses halaman catalog maupun viewer - redirect ke `/login`
- [x] REQ-02: Semua user terotentikasi bisa akses semua ebook Level 1
- [x] REQ-03: Semua ebook Level 2-3 ditampilkan sebagai premium / belum aktif
- [x] REQ-04: User yang mencoba akses ebook Level 2-3 diarahkan ke halaman upgrade
- [x] REQ-05: Pengecekan permission tetap terjadi di server-side untuk route viewer dan API stream
- [x] REQ-06: Role `admin` tetap bisa dipakai untuk bypass semua level bila owner membutuhkannya saat testing internal

---

## Acceptance Criteria

- [x] AC-01: Jika user belum login dan akses `/dashboard/*`, di-redirect ke `/login?redirect=<original-path>`
- [x] AC-02: Setelah login sukses, redirect mengikuti precedence di `01-auth.md`
- [x] AC-03: Ebook Level 2-3 di katalog menampilkan badge/ikon premium
- [x] AC-04: Klik ebook premium membawa user ke `/dashboard/upgrade`, bukan membuka PDF
- [x] AC-05: API route `/api/ebook/[id]/stream` return `403` untuk ebook Level 2-3 bagi user biasa
- [x] AC-06: User admin bisa membaca semua level untuk kebutuhan operasional/testing
...
- Proxy hanya cek auth untuk `/dashboard/*`
...
| 2026-04-23 | 2.6 | Tandai access control sebagai implemented, centang REQ/AC, dan sinkronkan istilah route protection ke `proxy.ts` |
